...
SV_WC_Framework_Bootstrap::instance()->register_plugin( '4.1.2', __( 'WooCommerce Braintree Gateway', 'woocommerce-gateway-braintree' ), __FILE__, 'init_woocommerce_gateway_braintree', array( 'is_payment_gateway' => true, 'minimum_wc_version' => '2.3', 'backwards_compatible' => '4.0.0' ) );
...
class WC_Braintree extends SV_WC_Payment_Gateway_Plugin {
	...
	public function add_admin_notices() {
		...
		if ( $credit_card_gateway->is_advanced_fraud_tool_enabled() && ! $this->get_admin_notice_handler()->is_notice_dismissed( 'fraud-tool-notice' ) ) {
			...
			$this->get_admin_notice_handler()->add_admin_notice(
				sprintf( __( 'Heads up! You\'ve enabled advanced fraud tools for Braintree. Please make sure that advanced fraud tools are also enabled in your Braintree account. Need help? See the %1$sdocumentation%2$s.', self::TEXT_DOMAIN ),
		...
		if ( isset( $paypal_settings['enabled'] ) && 'yes' == $paypal_settings['enabled'] ) {
			if ( isset( $paypal_settings['environment'] ) && 'production' == $paypal_settings['environment'] ) {
				...
				if ( 'no' === get_option( 'woocommerce_force_ssl_checkout' ) && ! $this->get_admin_notice_handler()->is_notice_dismissed( 'ssl-recommended-notice' ) ) {
					...
					$this->get_admin_notice_handler()->add_admin_notice( __( 'WooCommerce is not being forced over SSL -- Using PayPal with Braintree requires that checkout to be forced over SSL.', self::TEXT_DOMAIN ), 'ssl-recommended-notice' );
				}
			}
		}
	}
	...
	public function get_settings_link( $gateway_id = null ) {
		...
		return sprintf( '<a href="%s">%s</a>',
			...
			self::CREDIT_CARD_GATEWAY_ID === $gateway_id ? __( 'Configure Credit Card', self::TEXT_DOMAIN ) : __( 'Configure PayPal', self::TEXT_DOMAIN )
		);
	}
...
/**
 * The WC_Braintree global object, exists only for backwards compat
 *
 * @deprecated 2.2.0
 * @name $wc_braintree
 * @global WC_Braintree $GLOBALS['wc_braintree']
 */
$GLOBALS['wc_braintree'] = wc_braintree();
