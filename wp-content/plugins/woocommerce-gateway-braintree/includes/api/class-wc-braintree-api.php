 ...
 * @since 3.0.0
 */
class WC_Braintree_API extends SV_WC_API_Base implements SV_WC_Payment_Gateway_API {
	...
	protected function init_sdk() {
		...
		// configure
		Braintree_Configuration::environment( $this->get_gateway()->get_environment() );
		Braintree_Configuration::merchantId( $this->get_gateway()->get_merchant_id() );
		Braintree_Configuration::publicKey( $this->get_gateway()->get_public_key() );
		Braintree_Configuration::privateKey( $this->get_gateway()->get_private_key() );
	}
	...
	public function verify_csc( WC_Order $order ) {
		...
		if ( ! empty( $order->payment->nonce ) && ! empty( $order->payment->token ) ) {
			...
			if ( ! $result->transaction_approved() ) {
				throw new SV_WC_Payment_Gateway_Exception( __( 'The CSC for the transaction was invalid or incorrect.', WC_Braintree::TEXT_DOMAIN ) );
			}
		}
	}
	...
	protected function do_remote_request( $callback, $callback_params ) {
		...
		try {
			...
			$response = call_user_func_array( $callback, $callback_params );
	...
	protected function handle_response( $response ) {
		...
		if ( $response instanceof Exception ) {
			throw new SV_WC_API_Exception( $this->get_braintree_exception_message( $response ), $response->getCode(), $response );
		}
	...
	protected function get_braintree_exception_message( $e ) {
		...
		switch ( get_class( $e ) ) {
			...
			case 'Braintree_Exception_Authentication':
				$message = __( 'Invalid Credentials, please double-check your API credentials (Merchant ID, Public Key, Private Key, and Merchant Account ID) and try again.', WC_Braintree::TEXT_DOMAIN );
			break;

			case 'Braintree_Exception_Authorization':
				$message = __( 'Authorization Failed, please verify the user for the API credentials provided can perform transactions and that the request data is correct.', WC_Braintree::TEXT_DOMAIN );
			break;

			case 'Braintree_Exception_DownForMaintenance':
				$message = __( 'Braintree is currently down for maintenance, please try again later.', WC_Braintree::TEXT_DOMAIN );
			break;

			case 'Braintree_Exception_NotFound':
				$message = __( 'The record cannot be found, please contact support.', WC_Braintree::TEXT_DOMAIN );
			break;

			case 'Braintree_Exception_ServerError':
				$message = __( 'Braintree encountered an error when processing your request, please try again later or contact support.', WC_Braintree::TEXT_DOMAIN );
			break;

			case 'Braintree_Exception_SSLCertificate':
				$message = __( 'Braintree cannot verify your server\'s SSL certificate. Please contact your hosting provider or try again later.', WC_Braintree::TEXT_DOMAIN );
			break;
	...
	protected function get_new_request( $args = array() ) {
		...
		switch ( $args['type'] ) {
			...
			case 'transaction':
				...
				return new WC_Braintree_API_Transaction_Request( $this->order );

			case 'customer':
				$this->set_response_handler( 'WC_Braintree_API_Customer_Response' );
				return new WC_Braintree_API_Customer_Request();

			case 'payment-method':
				$this->set_response_handler( 'WC_Braintree_API_Payment_Method_Response' );
				return new WC_Braintree_API_Payment_Method_Request();
