...
class WC_Gateway_Braintree_PayPal extends WC_Gateway_Braintree {
	...
	public function __construct() {
		...
		parent::__construct(
			...
			array(
				'method_title'       => __( 'Braintree (PayPal)' ),
				'method_description' => __( 'Allow customers to securely pay using their PayPal account via Braintree.' ),
				'supports'           => array(
	...
	public function tweak_payment_methods_text( $translated_text, $raw_text, $text_domain ) {
		...
		if ( WC_Braintree::TEXT_DOMAIN === $text_domain ) {

			if ( 'Use a new bank account' === $raw_text ) {

				$translated_text = __( 'Use a new PayPal account', WC_Braintree::TEXT_DOMAIN );

			} elseif ( 'Bank Accounts' === $raw_text ) {

				$translated_text = __( 'PayPal Accounts', WC_Braintree::TEXT_DOMAIN );
			}
		}
	...
	public function tweak_my_payment_methods_delete_text( $actions, $token ) {
		...
		if ( $token->is_paypal_account() ) {
			$actions['delete']['name'] = __( 'Unlink', WC_Braintree::TEXT_DOMAIN );
		}
	...
	protected function do_paypal_transaction( WC_Order $order ) {
		...
		if ( $response->transaction_approved() ) {
			...
			// translators: %1$s - payment method title (e.g. PayPal), %2$s - transaction environment (either Sandbox or blank string), %3$s - type of transaction (either Authorization or Payment)
			$message = sprintf(
				__( '%1$s %2$s %3$s Approved', WC_Braintree::TEXT_DOMAIN ),
				$this->get_method_title(),
				$this->is_test_environment() ? __( 'Sandbox', WC_Braintree::TEXT_DOMAIN ) : '',
				$this->perform_credit_card_authorization() ? __( 'Authorization', WC_Braintree::TEXT_DOMAIN ) : __( 'Payment', WC_Braintree::TEXT_DOMAIN )
			);

			// adds the transaction id (if any) to the order note
			if ( $response->get_transaction_id() ) {
				// translators: %s - transaction ID
				$message .= ' ' . sprintf( __( '(Transaction ID %s)', WC_Braintree::TEXT_DOMAIN ), $response->get_transaction_id() );
			}
	...
	protected function get_saved_payment_token_order_note( $token ) {
		...
		return sprintf( __( 'PayPal Account Saved: %s', WC_Braintree::TEXT_DOMAIN ), $token->get_payer_email() );
	}
	...
	protected function get_default_title() {
		...
		return __( 'PayPal', WC_Braintree::TEXT_DOMAIN );
	}
	...
	protected function get_default_description() {
		...
		return __( 'Click the PayPal icon below to sign into your PayPal account and pay securely.', WC_Braintree::TEXT_DOMAIN );
	}
