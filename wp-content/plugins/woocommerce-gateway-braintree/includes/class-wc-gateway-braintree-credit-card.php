...
class WC_Gateway_Braintree_Credit_Card extends WC_Gateway_Braintree {
	...
	public function __construct() {
		...
		parent::__construct(
			...
			array(
				'method_title'       => __( 'Braintree (Credit Card)' ),
				'method_description' => __( 'Allow customers to securely pay using their credit card via Braintree.' ),
				'supports'           => array(
	 ...
	 *
	 * @since 3.0.0
	 * @see WC_Gateway_Braintree:enqueue_scripts()
	 * @return bool
	 */
	public function enqueue_scripts() {

		$enqueued = parent::enqueue_scripts();
	...
	protected function get_method_form_fields() {
		...
		$fields = array(
			...
			'fraud_settings_title' => array(
				'title' => __( 'Fraud Settings', WC_Braintree::TEXT_DOMAIN ),
				'type'  => 'title',
			),
			'fraud_tool'           => array(
				'title'    => __( 'Fraud Tool', WC_Braintree::TEXT_DOMAIN ),
				'type'     => 'select',
				'class'    => 'js-fraud-tool',
				'desc_tip' => __( 'Select the fraud tool you want to use. Basic is enabled by default and requires no additional configuration. Advanced requires you to enable advanced fraud tools in your Braintree control panel. To use Kount Direct you must contact Braintree support.', WC_Braintree::TEXT_DOMAIN ),
				'options'  => array(
					'basic'        => __( 'Basic', WC_Braintree::TEXT_DOMAIN ),
					'advanced'     => __( 'Advanced', WC_Braintree::TEXT_DOMAIN ),
					'kount_direct' => __( 'Kount Direct', WC_Braintree::TEXT_DOMAIN ),
				),
			),
			'kount_merchant_id'    => array(
				'title'    => __( 'Kount merchant ID', WC_Braintree::TEXT_DOMAIN ),
				'type'     => 'text',
				'class'    => 'js-kount-merchant-id',
				'desc_tip' => __( 'Speak with your account management team at Braintree to get this.', WC_Braintree::TEXT_DOMAIN ),
			),

			// 3D Secure
			'threed_secure_title' => array(
				'title'       => __( '3D Secure (Verified by Visa, MasterCard SecureCode)', WC_Braintree::TEXT_DOMAIN ),
				'type'        => 'title',
				'description' => sprintf( __( '3D Secure benefits cardholders and merchants by providing an additional layer of verification. %1$sLearn more about 3D Secure%2$s.', WC_Braintree::TEXT_DOMAIN ), '<a href="' . esc_url( $this->get_plugin()->get_documentation_url() ) . '#3d-secure' . '">', '</a>' ),
			),
			'threed_secure_enabled' => array(
				'title'       => __( '3D Secure', WC_Braintree::TEXT_DOMAIN ),
				'type'        => 'checkbox',
				'label'       => __( 'Enable 3D Secure (Visa and MasterCard cards only)', WC_Braintree::TEXT_DOMAIN ),
				'description' => __( 'You must contact Braintree support to add this feature to your Braintree account before enabling this option.', WC_Braintree::TEXT_DOMAIN ),
				'default'     => 'no',
			),
		);
	...
	protected function add_csc_form_fields( $form_fields ) {
		...
		$form_fields['require_csc'] = array(
			'title'   => __( 'Card Verification (CSC)', WC_Braintree::TEXT_DOMAIN ),
			'label'   => __( 'Display and Require the Card Security Code (CVV/CID) field on checkout', WC_Braintree::TEXT_DOMAIN ),
			'type'    => 'checkbox',
			'default' => 'yes',
		);
	...
	public function get_order( $order ) {
		...
		if ( $this->is_3d_secure_enabled() ) {
			...
			if ( SV_WC_Helper::get_post( 'wc_braintree_credit_card_3d_secure_verified' ) && ! empty( $order->payment->token ) && ! empty( $order->payment->nonce ) ) {
	...
	protected function do_credit_card_transaction( $order, $response = null ) {
		...
		if ( is_null( $response ) ) {
			...
			$response = $this->perform_credit_card_charge() ? $this->get_api()->credit_card_charge( $order ) : $this->get_api()->credit_card_authorization( $order );

			if ( $response->transaction_approved() ) {
				$order->payment->account_number = $response->get_masked_number();
				$order->payment->last_four      = $response->get_last_four();
				$order->payment->card_type      = SV_WC_Payment_Gateway_Helper::card_type_from_account_number( $response->get_masked_number() );
	...
	public function add_payment_gateway_transaction_data( $order, $response ) {
		...
		if ( $this->is_advanced_fraud_tool_enabled() && $response->has_risk_data() ) {
			$this->update_order_meta( $order->id, 'risk_id', $response->get_risk_id() );
			$this->update_order_meta( $order->id, 'risk_decision', $response->get_risk_decision() );
		}

		// add 3D secure data
		if ( $this->is_3d_secure_enabled() && $response->has_3d_secure_info() ) {
			$this->update_order_meta( $order->id, 'threeds_status', $response->get_3d_secure_status() );
		}
	}
	...
	public function adjust_fraud_script_tag( $url ) {
		...
		if ( SV_WC_Helper::str_exists( $url, 'braintree-data.js' ) ) {
	...
	public function validate_fields() {
		...
		if ( $is_valid && $this->is_3d_secure_enabled() && SV_WC_Helper::get_post( 'wc_braintree_credit_card_payment_nonce' ) ) {

			$error = false;

			try {

				$payment_method = $this->get_api()->get_payment_method_from_nonce( SV_WC_Helper::get_post( 'wc_braintree_credit_card_payment_nonce' ) );
				...
				if ( $payment_method->has_3d_secure_info() && ! $payment_method->get_3d_secure_liability_shifted() && ( $payment_method->get_3d_secure_liability_shift_possible() || $this->is_3d_secure_liability_shift_always_required() ) ) {
					$error = __( 'We cannot process your order with the payment information that you provided. Please use an alternate payment method.', WC_Braintree::TEXT_DOMAIN );
				}
				...
				$error = __( 'Oops, there was a temporary payment error. Please try another payment method or contact us to complete your transaction.', WC_Braintree::TEXT_DOMAIN );
			}
