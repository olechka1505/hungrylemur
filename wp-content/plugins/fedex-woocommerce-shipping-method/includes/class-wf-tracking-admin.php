...
class WF_Tracking_Admin_FedEx
{
	...
	function wf_add_tracking_info_to_email( $order, $sent_to_admin = false, $plain_text = false ) {
		...
		if( !empty( $shipment_result_array ) ) {
			echo '<h3>'.__( 'Shipping Detail', 'wf_fedEx_wooCommerce_shipping' ).'</h3>';
	...
	function display_api_message_table ( $tracking_info_api_array ) {
		...
		echo '<h3>'.__( self::TRACKING_TITLE_DISPLAY, 'wf_fedEx_wooCommerce_shipping' ).'</h3>';
		echo '<table class="shop_table wooforce_tracking_details">
			<thead>
				<tr>
					<th class="product-name">'.__( 'Shipment ID', 'wf_fedEx_wooCommerce_shipping' ).'<br/>('.__( 'Follow link for detailed status.', 'wf_fedEx_wooCommerce_shipping' ).')</th>
					<th class="product-total">'.__( 'Status', 'wf_fedEx_wooCommerce_shipping' ).'</th>
				</tr>
			</thead>
			<tfoot>';
		...
		foreach ( $tracking_info_api_array as $tracking_info_api ) {
			...
			if( '' == $tracking_info_api['api_tracking_status'] ) {
				$message = __( 'Unable to update real time status at this point of time. Please follow the link on shipment id to check status.', 'wf_fedEx_wooCommerce_shipping' );
			}
			else {
				$message = $tracking_info_api['api_tracking_status'];
			}
			echo '<td><span>'.__( $message, 'wf_fedEx_wooCommerce_shipping' ).'</span></td>';
			echo '</tr>';
		}
		echo '</tfoot>
		</table>';
	...
	function wf_admin_notice(){
		...
		switch ( $wftrackingmsg ) {
			case "0":
				echo '<div class="error"><p>'.self::SHIPPING_METHOD_DISPLAY.': '.__( 'Sorry, Unable to proceed.', 'wf_fedEx_wooCommerce_shipping' ).'</p></div>';
				break;
			case "4":
				echo '<div class="error"><p>'.self::SHIPPING_METHOD_DISPLAY.': '.__( 'Unable to track the shipment. Please cross check shipment id or try after some time.', 'wf_fedEx_wooCommerce_shipping' ).'</p></div>';
				break;
			case "5":
				$wftrackingmsg = get_post_meta( $post->ID, self::TRACKING_MESSAGE_KEY, true);
				if( '' != trim( $wftrackingmsg )) {
					echo '<div class="updated"><p>'.__( $wftrackingmsg, 'wf_fedEx_wooCommerce_shipping' ).'</p></div>';
				}
				break;
			case "6":
				echo '<div class="updated"><p>'.__( 'Tracking is unset.', 'wf_fedEx_wooCommerce_shipping' ).'</p></div>';
				break;
			case "7":
				echo '<div class="updated"><p>'.__( 'Tracking Data is reset to default.', 'wf_fedEx_wooCommerce_shipping' ).'</p></div>';
				break;
			default:
				break;
		}
	...
	function wf_add_tracking_metabox() {
		...
		add_meta_box( self::TRACKING_METABOX_KEY, __( self::TRACKING_TITLE_DISPLAY, 'wf_fedEx_wooCommerce_shipping' ), array( $this, 'wf_tracking_metabox_content' ), 'shop_order', 'side', 'default' );
	}
	...
	?>
				...
				<img class="help_tip" style="float:none;" data-tip="<?php _e( 'This field is Optional.', 'wf_fedEx_wooCommerce_shipping' ); ?>" src="<?php echo WC()->plugin_url();?>/assets/images/help.png" height="16" width="16" /><br>
				<input type="text" id="order_date_fedex" class="date-picker" value="<?php echo $shipment_source_data['order_date']; ?>"></p>
			</li>
			<li id="" class="wide">
				<a class="button button-primary woocommerce_shipment_fedex_tracking tips" href="<?php echo $tracking_url; ?>" data-tip="<?php _e( 'Save/Show Tracking Info', 'wf_fedEx_wooCommerce_shipping' ); ?>"><?php _e('Save/Show Tracking Info', 'wf_fedEx_wooCommerce_shipping'); ?></a>
			</li>
		</ul>
		<script>
