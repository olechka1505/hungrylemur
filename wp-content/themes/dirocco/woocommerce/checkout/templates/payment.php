<div class="col-xs-12">
    ...
    <form ng-submit="payment()">
        <div class="col-md-8 col-xs-12">
            <div class="col-xs-12">
                <label>Expiration date</label>
            </div>
            <div class="col-md-6 col-xs-12">
                <input type="text" maxlength="16" ng-model="paymentData.number" class="form-control" placeholder="Credit Card">
            </div>
            <div class="col-md-2 col-xs-12">
                <select ng-model="paymentData.month" class="form-control">
                    <option value="">Month</option>
                    <option value="{{key + 1}}" ng-repeat="(key, value) in month">{{value}}</option>
                </select>
            </div>
            <div class="col-md-2 col-xs-12">
                <select ng-model="paymentData.year" class="form-control">
                    <option value="">Year</option>
                    <option value="{{key}}" ng-repeat="(key, value) in years">{{value}}</option>
                </select>
            </div>
            <div class="col-md-2 col-xs-12">
                <input type="text" maxlength="4" ng-model="paymentData.cvc" class="form-control" placeholder="CVC">
            </div>
        </div>
